...
| `wslpg` | Liquidación primaria de granos | OK | sí · offline (fakes) + cassette homo (catálogos) | 200 | **ACTIVO (live)** | Catálogos `Consultar*` + `ConsultarUltNroOrden` **validados en vivo** (autorizado en WSASS). Fix: el parseo de catálogos esperaba la estructura al revés (rompía con >1 ítem) → `normalizar_lista_soap(.., "codigoDescripcion")`. `Dummy` gateado por el server (`common_001`); escrituras modeladas offline |
